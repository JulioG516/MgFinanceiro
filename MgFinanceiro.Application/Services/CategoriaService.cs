...
public class CategoriaService : ICategoriaService
{
    ...
    {
        _logger.LogInformation("Listando todas as categorias. Filtros - Tipo: {TipoCategoria}, Ativo: {StatusAtivo}", 
        ...
        try
        {
            ...
            _logger.LogInformation("Consulta de categorias realizada com sucesso. " +
                "Total encontrado: {TotalCategorias}, Tempo de execução: {TempoMs}ms", 
                responseList.Count, stopwatch.ElapsedMilliseconds);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Erro ao listar categorias. Filtros - Tipo: {TipoCategoria}, Ativo: {StatusAtivo}. " +
                "Tempo decorrido: {TempoMs}ms", 
                tipoCategoria?.ToString() ?? "Todos", statusCategoriaAtivo?.ToString() ?? "Todos", stopwatch.ElapsedMilliseconds);
            throw;
        }
    }
    ...
    {
        _logger.LogInformation("Iniciando atualização de status da categoria. ID: {CategoriaId}, NovoStatus: {NovoStatus}", 
            id, request.Ativo);
